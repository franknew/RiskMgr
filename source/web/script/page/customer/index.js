			...
			this._getData(id,function(data) {
				modal.show({
					...
					button: [{
						...
						style:'danger',
						callback:function() {
							if (!confirm('确认删除客户“'+data.Name+'”？')) {
								return ;
							}
							var dialog = this;
							ajax.post({
								url:'RiskMgr.Api.CustomerApi/Delete',
								form:this.form,
			...
			this._getData(id,function(data) {
				modal.show({
					...
					ok: function() {
						var dialog = this;
						ajax.post({
							url:'RiskMgr.Api.CustomerApi/Update',
							form:this.form,
							success:function(data, textStatus, jqXHR) {
								msg.success('编辑成功');
								dialog.close();
		...
		add:function(success) {
			modal.show({
				...
				ok: function() {
					var dialog = this;
					ajax.post({
						url:'RiskMgr.Api.CustomerApi/Add',
						form:this.form,
						success:function(data, textStatus, jqXHR) {
							msg.success('添加成功');
							dialog.close();
...
define.pack("./list",["jquery","risk/unit/ajax","risk/unit/route","risk/components/pager/index","./tmpl","./dialog"],function(require, exports, module){
	...
	var MOD = {
		initPage:function(params) {
			...
			route.on('click','add',function(ev) {//按钮：新增客户
				ev.preventDefault();
				dialog.add();
			});
		},
