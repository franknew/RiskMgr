...
;(function  () {
	...
	var iPad = agent.match(/(iPad).*OS\s([\d_]+)/),
		...
		iPod = agent.match(/(iPod).*OS\s([\d_]+)/),
		android = agent.match(/(Android)\s+([\d.]+)/)||agent.match(/Android/),
		wp = agent.match(/Windows Phone ([\d.]+)/),
		isMobile = iPad || iPhone || iPad || wp || android,
		isChrome = !!(/\bchrome\b/i.test(agent) || /\bsafari\b/i.test(agent));

	if (isMobile && isChrome) {	//仅支持手机、chrome、safari看
